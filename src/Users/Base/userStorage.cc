...
void userStorage::initialize(){
    ...
    container_waiting_for_remote_storage_Queue=new Container_JobQueue();
    dir_remote_storage.clear();
...
vector<StorageRequest*> userStorage::createFSforContainerJob (Container_jobBase* job, string opIp, string nodeSetName, int nodeId, int optionalID)
        {
        ...
        // Init ..
            ...
            if ((job->getFSSize() == 0) && (job->getPreloadSize() == 0)){
                ...
                throw cRuntimeError("The user %s[%i] has not defined an file system structure to its vms..", this->getUserName().c_str(), this->getUserId());
                    ...
                    int commId = allocateContainerJob(job);
                    job->setCommId(commId);
                // Create the requests for allocate preload or connect to remote storage..
                    requests_vector_storage = generateContainerStorageRequests (job, opIp, optionalID);
                // Send all requests for remote storage before change the state of vms to pending remote storage
                    for (reqIt = requests_vector_storage.begin();reqIt != requests_vector_storage.end(); reqIt++){
                        (*reqIt)->setNodeSetId(nodeSetName);
                        (*reqIt)->setNodeId(nodeId);
                ...
                // Move the job until the request will be atendeed..
                    ...
                    containerWaitingQueue->move_to_qDst(containerWaitingQueue->get_index_of_job(job->getId()),
                                               container_waiting_for_remote_storage_Queue,
                                               container_waiting_for_remote_storage_Queue->get_queue_size());
            }
...
vector<StorageRequest*> userStorage::generateContainerStorageRequests (Container_jobBase* job, string opIp, int pId)
        {
    ...
    // Define ..
        ...
        // The fs has remote routes
            if (hasRemoteFS(fsConfig)){
                ...
                // Initialize the values of the request
                    req->setOperation(CONTAINER_REQUEST_REMOTE_STORAGE);
                    req->setState(REQUEST_PENDING);
                    setUserToRequest(req);
                    if (pId == -1)
                        req->setPid(job->getJobId());
    ...
    //                // Create the connection
            ...
            if (hasLocalFS(fsConfig)) {
                ...
                // Initialize the values of the request
                    req->setOperation(CONTAINER_REQUEST_LOCAL_STORAGE);
                    setUserToRequest(req);
                    if (pId != -1){
                        req->setPid(pId);
                        req->setSPid(job->getJobId());
