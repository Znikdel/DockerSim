...
int Abstract_Remote_FS::getServerCellPosition (icancloud_App_IO_Message* sm){
...
void Abstract_Remote_FS::enqueueRequest(icancloud_Message* request){
	requests_queue.insert(requests_queue.end(), request);
}

icancloud_Message* Abstract_Remote_FS::popEnqueueRequest(){
	icancloud_Message* msg;

	msg = (*requests_queue.begin());
