...
void EnergyMeterStates::resetMemorizationVector (){
    bzero (nonAccumulatedTimeStates, (int)states.size()*sizeof(float));
}
...
void EnergyMeterStates::activateMemoVector(){
    if (simTime().dbl() != lastEvent.dbl()){
        double t = simTime().trunc(-3).dbl();
        nonAccumulatedTimeStates[actualStatePos]= t - lastEvent.trunc(-3).dbl();
        lastEvent = t;
    }
}
